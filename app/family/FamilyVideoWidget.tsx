...
function buildAIInterpretation(result: any): Interpretation {
    ...
    const narrative = getText(
        primaryDog?.narrative
    );
    ...
    let title =
        "Observed Emotional & Behavioral State";

    if (isFearful) {
        title =
            "Possible Fear or Discomfort Signals";
        ...
        paragraphs.push(
            `The analysis was most consistent with ${behavior ||
            "the observed behavioral state"
            }${descriptor
                ? ` — ${descriptor}`
                : ""
            }.`
    ...
    const processVideoFile = async (
        ...
        try {
            ...
            if (
                ...
                console.error(
                    "Prepare endpoint returned non-JSON response:",
                    responseText
                );

                throw new Error(
                    "The server returned an invalid upload preparation response."
                );
            }
            ...
            if (
                ...
                throw new Error(
                    "Server returned incomplete upload information."
                );
            }
            ...
            if (
                ...
                throw new Error(
                    `Video upload failed: ${uploadText ||
            ...
            const finalizeContentType =
                finalizeResponse.headers.get(
                    "content-type"
                );

            if (
                !finalizeContentType ||
                !finalizeContentType.includes(
                    "application/json"
                )
            ) {
                const responseText =
                ...
                console.error(
                    "Finalize endpoint returned non-JSON response:",
                    responseText
                );

                throw new Error(
                    "The server returned an invalid job creation response."
                );
            }
